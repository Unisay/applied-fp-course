...
handleRequest _db (ViewRq topic) =
  fmap (resp200Json (E.list encodeComment)) <$> DB.getComments _db topic
handleRequest _db ListRq =
  fmap (resp200Json (E.list encodeTopic)) <$> DB.getTopics _db
...
mkErrorResponse = \case
  UnknownRoute -> resp404 PlainText "Unknown Route"
  EmptyCommentText -> resp400 PlainText "Empty Comment"
  EmptyTopic -> resp400 PlainText "Empty Topic"
  DatabaseError err -> resp500 PlainText ("Database Error: " <> LBS.pack (show err))

...
runApplication :: IO ()
runApplication = runAppM prepareAppReqs >>= \case
...
prepareAppReqs = do
  ...
  db <- liftIO (DB.initDB (confDbFile conf)) >>= first DBInitErr . liftEither
  return (conf, db)
...
handleRequest :: DB.FirstAppDB -> RqType -> App Response

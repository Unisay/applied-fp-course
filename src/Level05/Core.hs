...
import Level05.Conf (Conf (dbFilePath), firstAppConfig)
...
newtype StartUpError = DBInitErr SQLiteResponse deriving (Show)
...
mkRequest rq = liftEither =<< case (pathInfo rq, requestMethod rq) of
...
mkErrorResponse (DBError _) = resp500 PlainText "Internal Server Error"
